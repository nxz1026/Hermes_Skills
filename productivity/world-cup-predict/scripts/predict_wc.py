...
def calculate_prediction(match, weights=None):
    ...
    # 比分预测 — 基于三向概率 + spread line
    spread_line = match.get("spread_home_line", "")
    hp_val = home_prob  # 加权后
    ap_val = away_prob
    dp_val = draw_prob_calc
    
    # 更合理的比分启发式
    if direction.startswith("平局"):
        if hp_val > 0.38:
            predicted_score = "1-1"
        else:
            predicted_score = "0-0"
    elif match['home'] in direction:
        if hp_val > 0.65:
            predicted_score = "3-0"
        elif hp_val > 0.50:
            predicted_score = "2-0"
        elif "-1.5" in spread_line:
            predicted_score = "2-0"
        elif "-0.5" in spread_line:
            predicted_score = "2-1"
        else:
            predicted_score = "1-0"
    else:
    ...
    # O/U 判断
    ou_total = match.get("total_over_close", "")
    if hp + ap > 0.7:
        ou = "Over " + (match.get("total_over_close","2.5"))
        ou_note = "高总分"
    else:
        ou = "Under " + (match.get("total_over_close","2.5"))
        ou_note = "低总分"
    ...
    return {
        ...
        "btts": "Yes" if hp > 0.3 and ap > 0.3 else "No",
        "reasoning_factors": {
...
def main():
    ...
    for p in predictions:
        print(f"  {p['match']} | {p['direction']} {p['stars']} | {p['predicted_score']} | {p['over_under']}", file=sys.stderr)
